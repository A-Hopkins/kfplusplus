...
namespace kfplusplus
{
  ...
  public:
                ...
                const linalg::Matrix<MEASUREMENT_DIM, MEASUREMENT_DIM>& measurement_noise)
    {
      ...
      // Innovation covariance S = H * P * H^T + R
      linalg::Matrix<MEASUREMENT_DIM, MEASUREMENT_DIM> S = measurement_matrix * PHt + measurement_noise;
     ...
     */
    void set_covariance(const linalg::Matrix<STATE_DIM, STATE_DIM>& covariance_matrix) { this->covariance = covariance_matrix; }

    /**
     * @brief Sets the state transition matrix.
     * @param transition_matrix The new state transition matrix.
     */
    void set_transition(const linalg::Matrix<STATE_DIM, STATE_DIM>& transition_matrix) { this->transition_matrix = transition_matrix; }

    /**
     * @brief Sets the control matrix.
     * @param control_matrix The new control matrix.
     */
    void set_control_matrix(const linalg::Matrix<STATE_DIM, CONTROL_DIM>& control_matrix) { this->control_matrix = control_matrix; }
  ...
  public:
    ...
    /**
     * @brief Performs the prediction step of the Extended Kalman Filter using a non-linear state transition.
     * 
     * @param state_transition_function Non-linear state transition function, f(x, u).
     * @param jacobian_transition Jacobian of the state transition function, J_f(x, u).
     * @param control The control vector, u (optional).
     */
    void predict_nonlinear(
      const std::function<linalg::Vector<STATE_DIM>(const linalg::Vector<STATE_DIM>&, const linalg::Vector<CONTROL_DIM>&)>& state_transition_function,
      const std::function<linalg::Matrix<STATE_DIM, STATE_DIM>(const linalg::Vector<STATE_DIM>&, const linalg::Vector<CONTROL_DIM>&)>& jacobian_transition,
