 ...
 * (EKF) for tracking an airplane.
 *
 * This program illustrates the use of the Extended Kalman Filter implemented in
 * the `kfplusplus` library for estimating the position and velocity of an
 * airplane in 2D space based on radar measurements. The EKF operates on a state
 * vector consisting of:
 *   - Position: x, y
 *   - Velocity: vx, vy
 *
 * The program uses a constant velocity motion model and simulates radar
 ...
 *
 * Steps:
 * 1. Initializes the EKF with a state transition matrix, process noise
 ...
 *
 * Example output demonstrates how the EKF handles non-linear measurements to
 * accurately estimate the airplane's position and velocity.
 */
...
int main()
{
  ...
  for (size_t i = 0; i < measurements.size(); ++i)
  {
    ...
    std::cout << "Measurement (range, bearing): (" << measurements[i](0) << ", "
              << measurements[i](1) << ")" << std::endl;
    ...
    std::cout << "Position uncertainty: " << std::sqrt(cov(0, 0)) << ", " << std::sqrt(cov(1, 1))
              << std::endl;
    std::cout << "Velocity uncertainty: " << std::sqrt(cov(2, 2)) << ", " << std::sqrt(cov(3, 3))
              << std::endl;
